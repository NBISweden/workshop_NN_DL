...
NB! In PyTorch, RNN input is a 3D tensor with shape *(seq, batch,
feature)* by default; set `batch_first=True` to reshape to *(batch,
seq, feature)*. Here, *seq* = sequence length, *batch* = batch size,
and *feature* = `input_size`, the number of expected features in the
input *x*.
