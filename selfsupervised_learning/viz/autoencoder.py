...
def autoencoder_visualization(width: int = PIXEL_WIDTH) -> None:
    ...
    The scene must have been rendered first via ``make ae``.
    ...
    matches = [
        p for p in media_root.rglob("AutoencoderScene.*")
        if p.suffix in _MIME_TYPES
    ]
    if not matches:
        raise FileNotFoundError(
            "No AutoencoderScene render found under media/. "
            "Run 'make ae' before rendering the slides."
        )
