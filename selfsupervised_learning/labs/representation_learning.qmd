...
Here we will use images of parasites from a [parasites dataset](https://universe.roboflow.com/graduao/sistema-para-analise-de-ovos-de-parasitas-em-amostra-de-agua-e-sangue). The dataset contain images of 8 different kinds of parasites extracted from microscopic images.

```{python}

# The dataset stores the images with the channel first, in this lab we flatten the images before sending them to the neural network, but the matplotlib visualization functions want's the channel axis first, so we add this transform so the visualization functions don't have to deal with it
...
#     root = "/home/eriky/src/workshop_NN_DL/selfsupervised_learning/labs/data/parasites/train", 
#     transform=train_transforms
# )
# dev_data = ImageFolder(
#     root = "/home/eriky/src/workshop_NN_DL/selfsupervised_learning/labs/data/parasites/valid", 
#     transform=eval_transforms
# )
# test_data = ImageFolder(
#     root = "/home/eriky/src/workshop_NN_DL/selfsupervised_learning/labs/data/parasites/test", 
#     transform=eval_transforms
# )
...
We've loaded the images as an ImageFolder dataset, which is a convenient way to load images organized in folders. Each folder corresponds to a class, and the images inside are the samples of that class. The `transform` argument allows us to specify how we want to preprocess the images. Here we resize them to grayscale 64x64 pixels and convert them to tensors.
...
for i, ax in enumerate(axes.flatten()):
    ...
    ax.set_title(train_data.labels[y])
    ax.axis('off')
plt.tight_layout()
plt.show()
```
