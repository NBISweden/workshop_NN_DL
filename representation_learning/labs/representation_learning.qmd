...
How close to the identity matrix is the result? Why might gradient descent converge to a non-orthogonal solution even though both are valid minimisers of the MSE objective?
...
- Reconstruction quality with `visualized_reconstruction`
- UMAP cluster separation with `visualize_latent_space`
...
- Does fine-tuning the encoder improve or hurt accuracy on `dev_data_b`?
...
1. Which self-supervised method comes closest to the supervised CNN on the transfer task?
2. Does CLIP's zero-shot representation outperform models trained on the same domain without labels? Why might that be surprising — or unsurprising?
3. Where would you invest effort if you had more unlabeled data but labeling was expensive?
